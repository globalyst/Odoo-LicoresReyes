...
class SaleOrderLine(models.Model):
	...
	is_offer = fields.Boolean(string="Is a Offer", default=False)
		
class SaleOrder(models.Model):
	_inherit = 'sale.order'

	offers_lines = fields.One2many('offer.sale.order.line','sale_order_ref', string="Offer lines")

	@api.multi
	def remove_offers(self):
		lines = self.env['sale.order.line'].search([('order_id', 'in', self.ids), ('is_offer', '=', True)])
		for line in lines:
			line.unlink()
			
	@api.multi
	def apply_offers(self):
		_logger.warning("SALE ORDER _apply_offers() INIT ")				
			...
			if offer_line.is_active:
				...
				for gift in offer.gift_id:
					for gift_line in gift.giftline_id:
						...
						if is_fix_price:
							_logger.warning("__apply_offers is_fix_price")	
							...
							if self.order_line:
								val['sequence'] = self.order_line[-1].sequence + 1
								sol = self.env['sale.order.line'].sudo().create(val)	
